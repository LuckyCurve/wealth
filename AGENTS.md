...
```
index.html   # 全部 HTML/CSS/JS (~4454 行)
...
- 测试文件：
  - `test/logic.test.js` — 纯逻辑（货币换算含 amountAtRates、金额格式化 money2、收益测算含 hasAnyRatedAsset/cashRatioPct/coveragePct、数据迁移、日期/月份、消费月份列表、消费标签分组下拉（tagFilterValue/parseTagFilter/expenseTagFilterGroups/hasTagFilterOption）、数据校验（isUntaggedItem/missingTagGroups/setTag/backfillTag）、排序状态机、备份决策与文案、汇率提示决策、颜色数学含 hexToRgba、图例选中态清理 pruneLegendSelected）。
  - `test/ui-wiring.test.js` — UI 接线契约：对 `index.html` 文本断言「设置=唯一入口」「io-modal 不含自动备份」「setTheme/setBackupFreq 接线各一处」「统一提示便条=右下角单一出口」「共享层单一来源（纯函数下沉/图表底座/对比 hero/openModal，内联副本回潮即报错）」「堆叠柱下钻与图例记忆（wireStackedBarDrill 单一来源/legendselectchanged 回灌 barChartBase/下钻弹窗复用 snapshot-modal）」「logic.js 导出清单无重复条目」「renderExpenses 先重建月份下拉再同步筛选值」「消费标签分组下拉（optgroup 分组头不可选/编码解码下沉 logic.js 不得内联重定义/syncExpenseTagFilter 唯一同步口/级联双下拉不回潮）」「收益覆盖标尺（骨架唯一/coveragePct+incomeGap+sumAssetIncomes 口径单一来源/分子口径切换接线/mo-badge 复用/可见性守卫形态/概念词禁词守卫——食利线·息可养支等不得回潮）」「自动备份等待汇率拉取完成再导出（DOMContentLoaded 捕获 fetchRates 的 promise 传入 maybeAutoBackup、函数内 await 排在 downloadBackupJSON 之前、失败不阻塞）」「分类管理回车添加标签（Enter 接线带 IME `isComposing` 守卫/按钮与回车共用 `addTagCall` 单一调用表达式/重绘后焦点续接且不写已脱离文档的旧 input/两侧收敛 addTagGeneric）」「消费数据校验（校验条唯一写入点 renderTagCheckBar/面板与三入口接线计数/chips 单一来源 tagPickerHtml 不得内联第二套/写标签只走 setTag+backfillTag 不得内联 tags 赋值/缺口口径 missingTagGroups 四处共用且资产侧 tagCheck=false 不接线/新建分类后先关弹窗再开面板）」等架构约束，防止单文件改动时接线被误改、旧入口回潮或重复实现各自漂移。
