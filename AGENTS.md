...
- 测试文件：
  - `test/logic.test.js` — 纯逻辑（货币换算含 amountAtRates、金额格式化 money2、收益测算、数据迁移、日期/月份、消费月份列表、排序状态机、备份决策与文案、汇率提示决策、颜色数学含 hexToRgba、图例选中态清理 pruneLegendSelected）。
  - `test/ui-wiring.test.js` — UI 接线契约：对 `index.html` 文本断言「设置=唯一入口」「io-modal 不含自动备份」「setTheme/setBackupFreq 接线各一处」「统一提示便条=右下角单一出口」「共享层单一来源（纯函数下沉/图表底座/对比 hero/openModal，内联副本回潮即报错）」「堆叠柱下钻与图例记忆（wireStackedBarDrill 单一来源/legendselectchanged 回灌 barChartBase/下钻弹窗复用 snapshot-modal）」等架构约束，防止单文件改动时接线被误改、旧入口回潮或重复实现各自漂移。
