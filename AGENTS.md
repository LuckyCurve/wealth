...
- **共享层单一来源（防重复实现各自漂移）** — 重复出现 ≥2 次的逻辑一律收敛：① 纯函数下沉 logic.js：`money2()`（千分位两位展示态，与 formatCNY/moneyStr 同族）、`hexToRgba()`（颜色数学归位）、`expenseMonths(expenses)`（消费月份列表，月份筛选与趋势维度下拉共用，防御脏 date）、`nextSortState()`（见三态排序）；② index.html 内视图模板/helper 单一来源：`chartTooltipBase(th)` + `barChartBase(th, dates, mode, legendData)`（历史净值/支出趋势两张堆叠柱的 tooltip 底座与 legend/grid/坐标轴样板，原先两处逐字重复；旭日图 tooltip 底座同样走 chartTooltipBase）、`renderCompareHero()`（快照对比/消费月对比弹窗顶部总变动 hero + 上下文行，含零变动分支）、`cmpBar()`/`cmpPctCell()`（对比表迷你量条与变动 % 单元格）；③ `openModal(id)` 与 `closeModal(id)` 成对，弹窗打开不再散落 `.style.display = ''`。ui-wiring 契约测试把守以上每一条（内联副本回潮即报错），修改对应逻辑时只改单一来源处。
...
- 测试文件：
  - `test/logic.test.js` — 纯逻辑（货币换算、金额格式化 money2、收益测算、数据迁移、日期/月份、消费月份列表、排序状态机、备份决策与文案、汇率提示决策、颜色数学含 hexToRgba）。
  - `test/ui-wiring.test.js` — UI 接线契约：对 `index.html` 文本断言「设置=唯一入口」「io-modal 不含自动备份」「setTheme/setBackupFreq 接线各一处」「统一提示便条=右下角单一出口」「共享层单一来源（纯函数下沉/图表底座/对比 hero/openModal，内联副本回潮即报错）」等架构约束，防止单文件改动时接线被误改、旧入口回潮或重复实现各自漂移。
