...
```
index.html   # 全部 HTML/CSS/JS (~4268 行)
...
- **汇率刷新** — `fetchRates()` 用 `cny.json` 端点且 `cache: 'no-store'`（CDN 7 天缓存导致普通刷新拿不到新汇率）；`pageshow`（bfcache 恢复）与 `visibilitychange`（切回标签页、上次拉取超 1 小时）触发刷新并重渲染；拉取成功后调用 `refreshVisibleCharts()` 同步重绘当前可见图表，并弹 info 便条「汇率已更新」。失败时回退保存的汇率，文案与语义色由 logic.js 纯函数 `rateFallbackNotice(hasCache)` 决策：有缓存 → info「使用缓存汇率」（中性金墨），无缓存 → error「汇率获取失败」（报警红）。旧版页头内联状态 `#rate-status` 已移除，提示统一走右下角便条。
...
- **共享层单一来源（防重复实现各自漂移）** — 重复出现 ≥2 次的逻辑一律收敛：① 纯函数下沉 logic.js：`money2()`（千分位两位展示态，与 formatCNY/moneyStr 同族）、`hexToRgba()`（颜色数学归位）、`expenseMonths(expenses)`（消费月份列表，月份筛选与趋势维度下拉共用，防御脏 date）、`nextSortState()`（见三态排序）、`pruneLegendSelected(selected, validNames)`（堆叠柱图例选中态清理，只保留仍存在的系列名，防换分类后旧隐藏项误伤新系列）、`amountAtRates(amount, currency, rates)`（显式汇率表折算 CNY，快照用 currencyRates/实时用 state.rates，堆叠柱聚合、快照对比、下钻明细共用此口径，防止图表色块值与弹窗明细漂移）、`coveragePct(cashMonthly, expectation)`/`incomeGap(amount, expectation)`（食利线覆盖率与收益−预期差额口径：食利线为盈余/缺口唯一渲染出口，报头不再重复渲染，防两处各算漂移）、`sumAssetIncomes(assets)`（两口径年/月/日收益合计：报头副行、收益摘要、食利线共用，防多处各算漂移）、`tagFilterValue(catId, tag)`/`parseTagFilter(value)`/`expenseTagFilterGroups(categories)`/`hasTagFilterOption(groups, value)`（消费标签分组下拉的选中值编码/解码、分类→分组结构、选中值有效性判定：编码取控制字符分隔符、解析取首个分隔符，DOM 侧不自拼口径，分类被删/标签改名后据此回落）；② index.html 内视图模板/helper 单一来源：`chartTooltipBase(th)` + `barChartBase(th, dates, mode, legendData, legendSelected)` + `wireStackedBarDrill(chartObj, metaGetter, openSegment)`（历史净值/支出趋势两张堆叠柱的 tooltip 底座与 legend/grid/坐标轴样板及点击下钻接线，原先两处逐字重复；legendSelected 可选参数回灌图例选中态，空对象不写入；旭日图 tooltip 底座同样走 chartTooltipBase）、`renderCompareHero()`（快照对比/消费月对比弹窗顶部总变动 hero + 上下文行，含零变动分支）、`cmpBar()`/`cmpPctCell()`（对比表迷你量条与变动 % 单元格）；③ `openModal(id)` 与 `closeModal(id)` 成对，弹窗打开不再散落 `.style.display = ''`。ui-wiring 契约测试把守以上每一条（内联副本回潮即报错），修改对应逻辑时只改单一来源处。
...
- 测试文件：
  - `test/logic.test.js` — 纯逻辑（货币换算含 amountAtRates、金额格式化 money2、收益测算含 hasAnyRatedAsset/cashRatioPct/coveragePct、数据迁移、日期/月份、消费月份列表、消费标签分组下拉（tagFilterValue/parseTagFilter/expenseTagFilterGroups/hasTagFilterOption）、排序状态机、备份决策与文案、汇率提示决策、颜色数学含 hexToRgba、图例选中态清理 pruneLegendSelected）。
  - `test/ui-wiring.test.js` — UI 接线契约：对 `index.html` 文本断言「设置=唯一入口」「io-modal 不含自动备份」「setTheme/setBackupFreq 接线各一处」「统一提示便条=右下角单一出口」「共享层单一来源（纯函数下沉/图表底座/对比 hero/openModal，内联副本回潮即报错）」「堆叠柱下钻与图例记忆（wireStackedBarDrill 单一来源/legendselectchanged 回灌 barChartBase/下钻弹窗复用 snapshot-modal）」「logic.js 导出清单无重复条目」「renderExpenses 先重建月份下拉再同步筛选值」「消费标签分组下拉（optgroup 分组头不可选/编码解码下沉 logic.js 不得内联重定义/syncExpenseTagFilter 唯一同步口/级联双下拉不回潮）」「食利线标尺（骨架唯一/coveragePct+incomeGap+sumAssetIncomes 口径单一来源/分子口径切换接线/mo-badge 复用/可见性守卫形态）」「自动备份等待汇率拉取完成再导出（DOMContentLoaded 捕获 fetchRates 的 promise 传入 maybeAutoBackup、函数内 await 排在 downloadBackupJSON 之前、失败不阻塞）」等架构约束，防止单文件改动时接线被误改、旧入口回潮或重复实现各自漂移。
